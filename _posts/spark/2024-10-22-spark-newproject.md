...
- **특징**:
  ...
  - 초기 설정이 복잡할 수 있지만, 관리 효율성과 확장성이 높습니다.
...
## 결론: 프로젝트의 규모가 작아 수동으로 설정할 수 있으므로 컨테이너 환경에서 실습해보도록 한다. 
...
docker run -d \
  ...
  --network host \
  -e SPARK_MODE=worker \
  -e SPARK_MASTER_URL=spark://<마스터노드IP>:7077 \
  bitnami/spark:latest
```

참고:
* 7077: 스파크 마스터 포트
* 8080: 스파크 마스터 웹 ui 포트
* 8081: 스파크 워커 웹 ui 포트

---

다음에는 **실험환경구축** 과정을 공부해보도록 하겠다. 